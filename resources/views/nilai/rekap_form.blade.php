        ...
        <div class="card shadow-xl border border-gray-100 bg-white">
            <form action="{{ route('nilai.rekap') }}" method="GET" class="space-y-6">
                ...
                <script>
                    ...
                    // FIX: gunakan 'change' dan 'input' agar dropdown-pick dari datalist
                    // maupun ketik manual keduanya tercapture.
                    function syncMahasiswa() {
                        const match = mahasiswaOptions.find(opt => opt.value === mahasiswaSearch.value);
                        mahasiswaHidden.value = match ? (match.dataset.id || '') : '';
                    }

                    function syncSemester() {
                        const match = semesterOptions.find(opt => opt.value === semesterSearch.value);
                        semesterHidden.value = match ? (match.dataset.value || '') : '';
                    }

                    mahasiswaSearch.addEventListener('input',  syncMahasiswa);
                    mahasiswaSearch.addEventListener('change', syncMahasiswa);
                    semesterSearch.addEventListener('input',   syncSemester);
                    semesterSearch.addEventListener('change',  syncSemester);

                    // FIX: validasi sebelum submit — pastikan hidden input terisi
                    mahasiswaSearch.closest('form').addEventListener('submit', function (e) {
                        syncMahasiswa();
                        syncSemester();
                        if (!mahasiswaHidden.value || !semesterHidden.value) {
                            e.preventDefault();
                            alert('Pilih mahasiswa dan semester yang valid dari daftar yang tersedia.');
                        }
                    });

                    // Sinkronisasi awal (untuk old() values)
                    syncMahasiswa();
                    syncSemester();
                </script>
            </form>
